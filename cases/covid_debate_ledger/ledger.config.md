...
project_name:        covid_debate
project_root:        <absolute path on disk>
deliverable:         epistemic case study (the recorded Rootclaim lab-leak debate)
source_of_truth:     markdown+quartz
the_reader:          a sceptical scientist AND an informed generalist
# The EXTRACTION.md source-type rows this case ingests (Anchor A): one ledger per
# voice, atomised by the row's structural unit. essay = Alexander's review;
# adjudication = the two judge decisions; probabilistic-analysis = the Bayesian
# write-ups; debate-transcript = the recorded debate turns.
source_types:        essay, adjudication, probabilistic-analysis, debate-transcript
...
# committed literature/units/<key>.units.json manifest, and its quote must be a
# verbatim span WITHIN that unit (tools/check_units.py). Makes the claim address
# mechanical (slug = locus) and the span bounded. Opt-in, NOT in the strict set
# (so a configured project is not forced to adopt it). A fully_enumerable=false
# (coarse PDF) manifest downgrades any required finding to a warning. Kept at
# 'warn' here because the case mixes clean-HTML and weak-PDF sources.
units_layer:         warn
