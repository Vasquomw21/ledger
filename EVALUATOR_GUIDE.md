...
- [COVID-origins](cases/covid_origins_ledger/) — a belief presented as a result, one disputed
  support, and two supporters marked as potentially correlated.
- [Eggs and cholesterol](cases/eggs_cholesterol_ledger/) — sixteen studies reduce to a handful of
  reused patient cohorts, and the detector raises three possible-dependence warnings, on both sides
  of the question.
- [LHC safety](cases/lhc_safety_ledger/) — the safety argument traced from its headline claim down
  to the exact sentence and reply it rests on.
- [Quantum computing for genomics](cases/quantum_genomics_ledger/) — a subject outside the author's
...
- [COVID debate](cases/covid_debate_ledger/) — a recorded public debate with no authored finding,
...
Read more: the authoring discipline ([`AGENTS.md`](AGENTS.md)), the interchange spec
([`spec/INTERCHANGE.md`](spec/INTERCHANGE.md)), the five-layer trust model
([`docs/integrity_framework.md`](docs/integrity_framework.md)), and the configuration reference
